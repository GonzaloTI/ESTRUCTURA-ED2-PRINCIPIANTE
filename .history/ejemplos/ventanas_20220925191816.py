...
class Ventana(object):
    ...
    def __init__(self,master):
        ...
        self.miMenu = Menu(self.master,text='Menu Principal') 
        self.barra = Menu(self.miMenu)
    # creamos los widget dentro del canvas       
    def inicializar_gui(self):
        Button(self.master,text='Dibujar',command=lambda: self.rectangulo()).pack()
        self.barra.add_command(label='Abrir' )
        self.miMenu.add_cascade(label='Archivo',menu=self.barra)
        self.master.config(menu=miMenu)
    def rectangulo(self):
